    ...
    return {
        ...
        refresh: useMutation({
            ...
            onError: (err, _newAttr, context) => {
                ...
                if (previousContact && odinId) {
                    updateCacheContact(queryClient, odinId, () => previousContact);
                }
                if (previousContact && id) {
                    updateCacheContact(queryClient, id, () => previousContact);
                }
            },
            onSettled: () => {
                if (odinId) {
                    invalidateContact(queryClient, odinId);
                }
                if (id) {
                    invalidateContact(queryClient, id);
                }
            },
        }),
    };
