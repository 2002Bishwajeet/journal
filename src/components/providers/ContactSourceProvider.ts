...
import { DotYouClient, type ImageContentType } from "@homebase-id/js-lib/core";
import {
...
export const fetchConnectionInfo = async (
  ...
  return {
    ...
    ...contactFromTransit,
    source:
      connectionContactData?.status === "connected" ? "contact" : "public",
  };
};
...
const queryRemoteAttributes = async (
  ...
  try {
    const [name, phone, email, location, birthday, photo] = await Promise.all([
      (
        await getProfileAttributesOverPeer(
          dotYouClient,
          odinId,
