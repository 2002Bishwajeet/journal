...
### Core
- [x] Setup React + Vite + PWA Service Worker + OPFS headers
- [x] Initialize PGlite with `document_updates` and `search_index` tables
- [x] Implement Homebase Auth & Drive creation (Types 605/706)

### Editor & Data
- [x] TipTap editor setup with Markdown transformers
- [x] Yjs provider connecting TipTap ↔ PGlite
- [x] Sync Engine (Homebase ↔ PGlite)
- [x] Image upload pipeline (Optimistic UI + Retry logic)

### AI & Search
- [x] WebLLM worker setup + OPFS model caching
- [x] Job Queue system for embeddings/suggestions
- [x] Hybrid Search UI (Title fallback logic)

### UI/UX
- [x] Sidebar & Navigation (Desktop vs Mobile layouts)
- [x] Mobile InputAccessoryView toolbar
- [x] Import/Export logic (Flattening + Tagging)
- [x] "Main" folder enforcement logic

### Remaining Polish Items
- [x] Inline grammar underlines UI (GrammarPlugin.ts with wavy underlines)
- [x] Autocomplete grey text suggestions ahead of cursor (AutocompletePlugin.ts)
- [x] Quick switcher (Cmd+K) - search modal with keyboard navigation
- [x] Mobile swipe gestures for delete/archive (enhanced with haptic feedback)